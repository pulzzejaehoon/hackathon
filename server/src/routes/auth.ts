...
async function loadUsers(): Promise<Array<{ id: number; email: string; password_hash: string }>> {
  try {
    await ensureDataDir();
...
// Save users to file
async function saveUsers(users: Array<{ id: number; email: string; password_hash: string }>) {
  try {
    await ensureDataDir();
