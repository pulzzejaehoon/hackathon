...
router.get('/events/list', async (req, res) => {
    try {
        ...
        const { calendarId = 'primary', timeMin, timeMax, maxResults = 10, orderBy = 'startTime', singleEvents = 'true', q } = req.query;
        const api = await callInteractorApi({
            account,
            connector: 'googlecalendar-v1',
...
router.post('/events/create', async (req, res) => {
    try {
        const account = req.user?.email;
        const { calendarId = 'primary', summary, start, end, description, location, attendees } = req.body || {};
        if (!account)
            return res.status(401).json({ ok: false, error: 'Unauthorized: missing user context' });
